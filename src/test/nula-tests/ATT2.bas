...
DEFFNl2n(L%,M%):LOCALP%:P%=PAL%(L%)AND&F
...
FF%=F%((PAL%(C%)AND&F)EOR7)<>0:FB%=F%((PAL%(C%AND&C)AND&F)EOR7)<>0
N$=FNl2n(C%,TRUE)+"/"+FNl2n(C%AND&C,TRUE)
N2$=FNl2n(C%,NOTFF%)+"/"+FNl2n(C%AND&C,NOTFB%)
IFN2$<>N$THENN$=N$+"!"+N2$
=N$
...
DATA &09,&2E,&CF,&D8:REM REMAP LOG->PHYS PALLETTE
DATA -1
DATA &FFFF228F,&FFFF229F
DATA -1
...
DATA "Test palette4 Flash"
DATA -1
DATA &FFFF2280,&FFFF229A
DATA -1
