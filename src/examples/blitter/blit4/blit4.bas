...
!BLIT_ADDR_E=&30000:REM save screen contents here
...
N%=N%-1
REPEAT
 ?BLIT_BLITCON=BLITCON_EXEC_B+BLITCON_EXEC_D: REM restore data from channel B to channel D
 ?BLIT_FUNCGEN=&CC:REM B
 ?BLIT_WIDTH=S_W%(N%)-1
 ?BLIT_HEIGHT=S_H%(N%)
 ?BLIT_SHIFT_A=0
 ?BLIT_MASK_FIRST=&FF:?DMAC_MASK_LAST=&FF:?DMAC_DATA_A=&FF:REM no mask
 !BLIT_STRIDE_B = S_W%(N%)
 !BLIT_STRIDE_C = SCR_CHAR_ROW% 
...
?BLIT_BLITCON=BLITCON_EXEC_A+BLITCON_EXEC_B+BLITCON_EXEC_C+BLITCON_EXEC_D+BLITCON_EXEC_E: REM bitmap from B, original screen C, write screen D
...
S_SA%(N%)=!BLIT_ADDR_C:S_RA%(N%)=!DMAC_ADDR_E:S_W%(N%)=?DMAC_WIDTH+1:S_H%(N%)=?DMAC_HEIGHT
